...
express.Router() is used to create modular rout handling in Express.js applications. It allows you to split routs into separate files or modules so that they can be used in different parts of the application or be reused. For example you can have separate files for user and products routes so that you can be better organized and have a cleaner code structure. 
...
- How would you implement error handling in the Express routes to ensure that any issues (such as file not found or server errors) are appropriately handled? Provide an example.
   ...
   I would implement error handling in the route itself suchas if I am reading a file I would handle the error of the file not being found or not being able to be read in the routes code. Or for example if an invalid input is givin that can be directly handled in the routes code with try-catch and if else statements. Finally I would create error handling middleware to catch any uncaught exceptions in my routes. 
---
...
This line dynamically sets the port to where its being hosted (ex. AWS) and also will default to port 8081 if that fails so that the application can be run locally. Being able to run the code locally on port 8081 is beneficial for production environments as we as the developers will be able to run the web application locally while we are developing it to deisgn our webpages and test our backend code. 
